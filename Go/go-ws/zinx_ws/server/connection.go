...
// 初始化链接服务
func NewConnection(server iserver.IServer, wsSocket *websocket.Conn, connId uint64, msgHandler iserver.IMsgHandle) *Connection {
	c := &Connection{
		Server:    server,
		Conn:      wsSocket,
		connId:    connId,
...
func (c *Connection) readLoop() {
	...
	for {
		...
		if _, ok := MsgJon["msgId"]; ok {
			message := NewMsg(MsgJon["msgId"].(string), msgType, msgData)
			//得到当前客户端请求的Request数据
			req := Request{
				conn: c,
				msg:  message,
